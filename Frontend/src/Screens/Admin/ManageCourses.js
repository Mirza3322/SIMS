...
export default function ManageCourse() {
  ...
  const [selectedClass, setSelectedClass] = useState('');
   ...
   // Map classInfo array to create picker items
  const classPickerItems = admin.classes.map((classItem) => (
    <Picker.Item key={classItem._id} label={classItem.name} value={classItem.id} />
  ));
  ...
  const { error,isDeleted, isAuthenticated_for_class_register, admin } = useSelector((state) => state.admin);
  ...
  useEffect(() => {
    ...
    if (isDeleted) {
      ...
      // Refresh the screen when a class is deleted
